...
# Introduction
How hard it is to create a data analysis workflow, deploy it and distribute it? sometimes it's quite hard. For example, here is standford huge seq pipeline. Each node represents a tool with it's own dependencies, written in different languanges. And a working analysis flow could as comlicated as shown. 
...
* Imporssible to reproduce a flow
...
In this chapter we will work through some real examples to write R command line tools, learn to use docopt starndard, describe your tools with rabix and creating tools with R interface etc. 



## What is Command Line Tool

Most of us are already familiar with command line tools, tons of unix tools and bioinformaits tools have command line interface, we give the command line tool parameters to launch the appplication and do some work for us. 

You tools may also have multiple subcommands, for examples, `samtools` 
...
* Note that if your tool has multiple subcommands, you will need to create a description in the graphical editor for each subcommand, saving each one as a new 'tool'. Each subcommand description should reference __the same Docker image__, which contains the tool. So your `view` or `sort` tool should reference the same `samtools` docker image. 
...
* The executable name of the subcommand, and any other part of the command that is preset for every execution of the tool as you describe it, is entered in the field _Base Command_. So `samtools sort` will be your base command. 

Each command comes with its own parameteres, for example, let's take a look for `samtools sort`
...
### Interface
You don't need to know about JSON, YAML or any languange, the most easy way is that you can simply start describing your tools and create workflows with the graphical user interface. 
...
Now please click the "+Create" button, it will ask you to save the tools to the speicific repos, you can create one as well. 
